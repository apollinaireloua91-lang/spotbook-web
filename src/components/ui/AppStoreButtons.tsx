...
export default function AppStoreButtons({ size = 'default' }: { size?: 'default' | 'large' }) {
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {/* Primary — App Store with neon gradient */}
      <motion.a
        ...
        whileTap={{ scale: 0.98 }}
        className={`btn-primary ${h} !px-6 !py-0 flex items-center gap-3 !rounded-2xl relative`}
      >
      ...
      {/* Secondary — Google Play as glass card */}
      <motion.a
        ...
        whileHover={{ scale: 1.03, y: -2 }}
        whileTap={{ scale: 0.98 }}
        className={`${h} px-6 rounded-2xl flex items-center gap-3 text-white bg-brand-card/50 border border-brand-green/40 backdrop-blur-md transition-all hover:border-brand-green-light/70 hover:shadow-neon hover:bg-brand-card/70`}
      >
        <svg width="26" height="26" viewBox="0 0 24 24" fill="currentColor" className="text-brand-green-light">
        ...
        <div className="text-left">
          <div className="text-[10px] opacity-70 leading-none">Disponible sur</div>
